...
- 消息持久化（SQLite）：重启后群组和聊天记录保留，重新进入群组时成员群组自动重连、创建者群组自动恢复服务（加入凭证已持久化，无需重新输入）
...
在控制台窗口按 **Ctrl+C** 可干净退出程序（会正常保存数据、释放端口）。该方式仅对控制台运行有效（`python main.py` 或 `run.bat`）；打包为无控制台的 exe 后没有控制台通道，请改用托盘菜单“退出”结束程序。
